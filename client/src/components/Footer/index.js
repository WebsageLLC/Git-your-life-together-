import React from 'react';

const Footer = () => {
    return (
        <div className="footer-clean col-sm-12">
            <footer>
                <div className="container ">
                    <div className="row justify-content-center">
                        <div className="col-sm-4 col-md-2 item">
                            <p className="copyright">© 2023, Developed By:</p>
                        </div>
                        <div className="col-sm-4 col-md-2 item">
                            <h3>Jessica Ortega </h3>
                            <ul>
                                <li><a href="https://github.com/JessicaIOrtega">GitHub</a></li>
                                <li><a href="mailto: jes.ortega7@gmail.com">Email</a></li>
                            </ul>
                        </div>
                        <div className="col-sm-4 col-md-2 item">
                            <h3>Camelia Benavides </h3>
                            <ul>
                                <li><a href="https://github.com/cameliabenavides10">GitHub</a></li>
                                <li><a href="mailto: cameliabenavides@gmail.com">Email</a></li>
                            </ul>
                        </div>
                        <div className="col-sm-4 col-md-2 item">
                            <h3>Michael Escamilla</h3>
                            <ul>
                                <li><a href="https://github.com/mescamilla1114">GitHub</a></li>
                                <li><a href="mailto: mescamilla1114@gmail.com">Email</a></li>
                            </ul>
                        </div>
                        <div className="col-sm-4 col-md-2 item">
                            <h3>Brett Nachman</h3>
                            <ul>
                                <li><a href="https://github.com/brettnachman">GitHub</a></li>
                                <li><a href="mailto: brettnachman@gmail.com">Email</a></li>
                            </ul>
                        </div>
                        <div className="col-sm-4 col-md-2 item">
                            <h3>Stephen Totten</h3>
                            <ul>
                                <li><a href="https://github.com/Crothos">GitHub</a></li>
                                <li><a href="mailto: s.g.totten@gmail.com">Email</a></li>
                            </ul>
                        </div>

                    </div>
                </div>
            </footer>
        </div>

    );
};

export default Footer;
